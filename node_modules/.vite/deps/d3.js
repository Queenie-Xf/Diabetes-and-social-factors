...
function nest(values, map5, reduce3, keys) {
...
function ticks(start2, stop, count4) {
  ...
  if (reverse3) {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks3[i] = (i2 - i) / -inc2;
    else for (let i = 0; i < n; ++i) ticks3[i] = (i2 - i) * inc2;
  } else {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks3[i] = (i1 + i) / -inc2;
    else for (let i = 0; i < n; ++i) ticks3[i] = (i1 + i) * inc2;
  }
  return ticks3;
}
...
function quickselect(array5, k, left2 = 0, right2 = Infinity, compare) {
  k = Math.floor(k);
  left2 = Math.floor(Math.max(0, left2));
  right2 = Math.floor(Math.min(array5.length - 1, right2));
  if (!(left2 <= k && k <= right2)) return array5;
  ...
  while (right2 > left2) {
    if (right2 - left2 > 600) {
      ...
      quickselect(array5, k, newLeft, newRight, compare);
    }
    const t = array5[k];
    let i = left2;
    let j = right2;
...
function thresholdFreedmanDiaconis(values, min4, max5) {
  const c5 = count(values), d = quantile2(values, 0.75) - quantile2(values, 0.25);
...
function selectAll_default3(select) {
  ...
  for (var groups3 = this._groups, m3 = groups3.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group3 = groups3[j], n = group3.length, node, i = 0; i < n; ++i) {
      if (node = group3[i]) {
        for (var children2 = select.call(node, node.__data__, i, group3), child, inherit2 = get2(node, id2), k = 0, l = children2.length; k < l; ++k) {
...
function brush(dim) {
  ...
  function started(event) {
    ...
    var that = this, type2 = event.target.__data__.type, mode3 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent4 = state.extent, selection2 = state.selection, W = extent4[0][0], w0, w1, N = extent4[0][1], n0, n1, E = extent4[1][0], e0, e1, S = extent4[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
  ...
  brush2.extent = function(_) {
    return arguments.length ? (extent3 = typeof _ === "function" ? _ : constant_default3(number22(_)), brush2) : extent3;
  };
  brush2.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default3(!!_), brush2) : filter4;
  };
...
function chord(directed, transpose3) {
  ...
  function chord2(matrix) {
    var n = matrix.length, groupSums = new Array(n), groupIndex = range2(0, n), chords = new Array(n * n), groups3 = new Array(n), k = 0, dx;
    ...
    dx = k ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups3[i] = { index: i, startAngle: x06, endAngle: x3, value: groupSums[i] };
        } else {
...
function bisector2(f) {
  ...
  if (f.length !== 2) {
    ...
    compare1 = f === ascending3 || f === descending2 ? f : zero2;
    compare2 = f;
    delta = f;
  }
  function left2(a3, x3, lo = 0, hi = a3.length) {
...
function tickSpec2(start2, stop, count4) {
  ...
  if (i2 < i1 && 0.5 <= count4 && count4 < 2) return tickSpec2(start2, stop, count4 * 2);
  return [i1, i2, inc2];
}
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    function stitch(line) {
      var start2 = [line[0][0] + x3, line[0][1] + y3], end = [line[1][0] + x3, line[1][1] + y3], startIndex = index4(start2), endIndex = index4(end), f, g;
  ...
  contours.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default5(slice4.call(_)) : constant_default5(_), contours) : threshold2;
  };
...
function density_default() {
  ...
  density.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default5(slice4.call(_)) : constant_default5(_), density) : threshold2;
  };
...
function collinear2(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a3 = 2 * triangles[i], b = 2 * triangles[i + 1], c5 = 2 * triangles[i + 2], cross3 = (coords[c5] - coords[a3]) * (coords[b + 1] - coords[a3 + 1]) - (coords[b] - coords[a3]) * (coords[c5 + 1] - coords[a3 + 1]);
...
function link_default(links) {
  var id2 = index3, strength = defaultStrength, strengths, distance = constant_default6(30), distances, nodes, count4, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function rangeContains(range5, x3) {
  return range5[0] <= range5[1] ? range5[0] <= x3 && x3 <= range5[1] : x3 < range5[0] || range5[1] < x3;
}
...
function polygonContains_default(polygon, point2) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      ...
      sum4.add(atan2(k * sign2 * sin2(absDelta), cosPhi03 * cosPhi1 + k * cos2(absDelta)));
...
function fitExtent(projection2, extent3, object2) {
  return fit(projection2, function(b) {
    var w = extent3[1][0] - extent3[0][0], h = extent3[1][1] - extent3[0][1], k = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x3 = +extent3[0][0] + (w - k * (b[1][0] + b[0][0])) / 2, y3 = +extent3[0][1] + (h - k * (b[1][1] + b[0][1])) / 2;
...
function computeUpperHullIndexes(points) {
  ...
  let size = 2, i;
  for (i = 2; i < n; ++i) {
    while (size > 1 && cross_default2(points[indexes3[size - 2]], points[indexes3[size - 1]], points[i]) <= 0) --size;
    indexes3[size++] = i;
  }
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection2, k, p, event) {
    zoom.transform(selection2, function() {
      var e = extent3.apply(this, arguments), t0 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t0.invert(p02), k1 = typeof k === "function" ? k.apply(this, arguments) : k;
  ...
  function dblclicked(event, ...args) {
    if (!filter4.apply(this, arguments)) return;
    var t0 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t0.invert(p02), k1 = t0.k * (event.shiftKey ? 0.5 : 2), t1 = constrain(translate(scale2(t0, k1), p02, p1), extent3.apply(this, args), translateExtent);
    noevent_default3(event);
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default9(!!_), zoom) : filter4;
  };
  zoom.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default9(!!_), zoom) : touchable;
  };
  zoom.extent = function(_) {
    return arguments.length ? (extent3 = typeof _ === "function" ? _ : constant_default9([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom) : extent3;
  };
