...
function blurfImage(radius) {
  ...
  return (T, S, start, stop, step) => {
    ...
    blur4(T, S, start + 0, stop + 0, step);
    blur4(T, S, start + 1, stop + 1, step);
    blur4(T, S, start + 2, stop + 2, step);
    blur4(T, S, start + 3, stop + 3, step);
  };
}
...
function locale_default(locale3) {
  ...
  function newFormat(specifier) {
    ...
    var fill = specifier.fill, align = specifier.align, sign = specifier.sign, symbol = specifier.symbol, zero4 = specifier.zero, width = specifier.width, comma = specifier.comma, precision = specifier.precision, trim = specifier.trim, type = specifier.type;
...
function loggish(transform) {
  ...
  scale.tickFormat = (count3, specifier) => {
    ...
    const k = Math.max(1, base * count3 / scale.ticks().length);
    return (d) => {
...
var e22 = Math.sqrt(2);
function tickSpec2(start, stop, count3) {
  ...
  if (i2 < i1 && 0.5 <= count3 && count3 < 2) return tickSpec2(start, stop, count3 * 2);
  return [i1, i2, inc];
}
function tickIncrement2(start, stop, count3) {
  stop = +stop, start = +start, count3 = +count3;
  return tickSpec2(start, stop, count3)[2];
}
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function ticks3(start, stop, count3) {
    ...
    const ticks4 = interval ? interval.range(start, +stop + 1) : [];
  ...
  function tickInterval(start, stop, count3) {
    ...
    if (i === tickIntervals.length) return year.every(tickStep2(start / durationYear, stop / durationYear, count3));
    if (i === 0) return millisecond.every(Math.max(tickStep2(start, stop, count3), 1));
...
function calendar(ticks3, tickInterval, year, month, week, day, hour, minute, second2, format2) {
  ...
  scale.copy = function() {
    return copy(scale, calendar(ticks3, tickInterval, year, month, week, day, hour, minute, second2, format2));
  };
  return scale;
}
function time() {
...
//# sourceMappingURL=chunk-T2GCAMA6.js.map
